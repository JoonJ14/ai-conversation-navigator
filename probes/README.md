...
1. `node probes/build-perf-probe.js` → `probes/acn-perf-probe.user.js`.
2. In Tampermonkey: **disable** the regular "AI Conversation Navigator" script
   ...
   entry, "AI Conversation Navigator (PERF PROBE)", version `12.3-perf1`.
3. Open the conversation, keep the tab **visible** throughout, open the DevTools
   console and filter on `ACN-PERF`.
4. Confirm the identity line (the DEC-027 guard — stop if it is missing or
   `realm=` is not `GM-sandbox`):
   `[ACN-PERF] probe build perf1 over ACN v12.3; perf.now granularity ~X ms; visibility=visible; realm=GM-sandbox`
5. Summary dot → auto-generate → **run#1** (expect the freeze here; let it
   finish). Click **Regenerate Summary** → **run#2**. Tools → Summary export →
   **run#3** with `trigger=export` — the double-run confirmation.
