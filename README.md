...
Demo Video: https://www.youtube.com/watch?v=4Viu418UKwc
...
- **🌐 i18n** — Korean language support. **Translation coverage is partial, and this line has twice promised more than the code delivers.** What actually happens on a language switch: dot labels and panel headers update live; **translated** strings built at injection apply after a page refresh, which the in-app toast tells you. **Untranslated strings never change, on any refresh** — most toasts are English literals (22 of 32 call sites), as are several Bookmarks controls. (An older version promised that *all* labels update live — they do not, and never did; a second version said "everything else applies after a page refresh", which is still false for any string that was never wired to the translation table in the first place. `docs/SETTINGS.md` §"Scope of translation" carries the measured, dated breakdown.) Some strings stay English on purpose — `/Commands` reads better untranslated, and three smaller surfaces are English by owner preference rather than oversight. Since **v12.7** the Summary's **topics and conversation map** read Korean too — before that its tokenizer kept only `a-z0-9`, so a Korean conversation produced zero tokens and every content-derived Summary section was silently empty. **Key Points is the exception and is still unavailable in Korean**: it matches a set of English phrase patterns, which no tokenizer change reaches. **v12.7a** additionally translated the **Tools** panel (Image Gallery, 파일 내보내기 and its three export options) and the **Plan usage** panel — thirteen Korean strings existed in the code but were never actually read, so those panels had rendered English regardless of the language setting. `/Commands` stays English on purpose: "slash command" reads better untranslated. (The tokenizer fix covers English and Korean specifically; Japanese and Chinese are not space-separated and would need word segmentation, not a wider character class.)
