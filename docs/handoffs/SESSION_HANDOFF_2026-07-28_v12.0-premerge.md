> **ARCHIVED — PREDECESSOR HANDOFF.** Superseded by `HANDOFF.md` at the repo root (session of
> 2026-07-29: v12.1 legacy bookmark recovery). Still authoritative for the **v12.0 pre-merge
> hardening arc** — the two load-path CRITICALs and their reproductions, the 24-round Codex
> cycle and its provenance arithmetic, and the attachment-headed Q#1 regression that was
> diagnosed and fixed here. NOT authoritative for current state, suite counts, or next steps.
